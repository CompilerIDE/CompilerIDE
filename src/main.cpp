...
bool LSPClient::startServer(const QString &clangdPath, const QString &compileCommandsDir)
{
    if (m_process)
    {
        stopServer();
    ...
    {
        args << "--compile-commands-dir=" + QDir::toNativeSeparators(compileCommandsDir);
    }
...
void LSPClient::openDocument(const QString &uri, const QString &text)
{
    ...
    textDoc["version"] = ++m_docVersions[uri];
    textDoc["text"] = text;
...
void LSPClient::onReadyRead()
{
    ...
    while (true)
    {
        int idx = m_buffer.indexOf("Content-Length: ");
        if (idx == -1)
        {
            break;
        }
        ...
        int length = m_buffer.mid(start, end - start).toInt();
        ...
        QByteArray jsonData = m_buffer.mid(bodyStart, length);
...
{
    emit serverError(QString("clangd error: %1").arg(error));
}
...
void LSPClient::sendMessage(const QJsonObject &msg)
{
    ...
    m_process->write(header + json);
}

void LSPClient::parseMessage(const QByteArray &data)
{
    QJsonParseError err;
...
void LSPClient::handleNotification(const QJsonObject &notif)
{
    ...
    if (method == "textDocument/publishDiagnostics")
    {
        ...
        for (const QJsonValue &v : diags)
        {
            ...
            int sev = d["severity"].toInt();
            if (sev == 1)
            {
                diag.severity = LSPDiagnostic::Error;
            }
            else if (sev == 2)
            {
                diag.severity = LSPDiagnostic::Warning;
            }
            else if (sev == 3)
            {
                diag.severity = LSPDiagnostic::Info;
            }
            else
            {
                diag.severity = LSPDiagnostic::Hint;
            }
            out.append(diag);
        }
...
void CodeEditor::setLspDiagnosticSelections(const QList<QTextEdit::ExtraSelection> &selections)
{
    lspDiagnosticSelections = selections;
    updateExtraSelections();
}
...
QString Debugger::createDebugBatchFile(const QString &executablePath)
{
    ...
    if (batFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QString pathSetting = "set PATH=\"" + mingwBin + "\";%PATH%";
...
static QString formatWin32Error(DWORD code = GetLastError())
{
    ...
    return QStringLiteral("Win32 error %1: %2").arg(code).arg(message);
}

static QString formatHresult(HRESULT hr)
{
...
static QString makeCommandLine(const QString &program, const QStringList &arguments)
{
    ...
    parts << quoteWindowsArgument(program);

    for (const QString &arg : arguments)
    {
        parts << quoteWindowsArgument(arg);
    }

    return parts.join(QLatin1Char(' '));
}

static QVector<wchar_t> toWideBuffer(const QString &text)
{
    QVector<wchar_t> result(text.size() + 1);
    text.toWCharArray(result.data());
    result[text.size()] = L'\0';
    return result;
}

static int queryWindowsBuildNumber()
{
...
static QString quotePowerShellLiteral(const QString &text)
{
...
public:
    explicit OfflineRequestInterceptor(QObject *parent = nullptr)
...
class TerminalWebPage final : public QWebEnginePage
{
    Q_OBJECT

public:
    explicit TerminalWebPage(QWebEngineProfile *profile, QObject *parent = nullptr)
...
protected:
                                 ...
                                 bool isMainFrame) override
    {
        ...
        return scheme == QStringLiteral("qrc") ||
...
class PowerShellPty final : public QObject
{
    Q_OBJECT
    Q_PROPERTY(int windowsBuild READ windowsBuild CONSTANT)
...
public:
    ...
    ~PowerShellPty() override
    {
        stop();
    }

    int windowsBuild() const
    {
        return m_windowsBuild;
    }

    void setInitialWorkingDirectory(const QString &dir)
    {
        Q_UNUSED(dir)
        m_initialWorkingDirectory = QDir::toNativeSeparators(QDir::homePath());
    }
...
public slots:
    ...
    void start(int cols, int rows)
    {
        ...
        if (!CreatePipe(&ptyInputRead, &hostInputWrite, &sa, 0))
        {
            emitError(QStringLiteral("CreatePipe input failed: %1").arg(formatWin32Error()));
            return;
        }

        if (!CreatePipe(&hostOutputRead, &ptyOutputWrite, &sa, 0))
        {
            CloseHandle(ptyInputRead);
            CloseHandle(hostInputWrite);
            emitError(QStringLiteral("CreatePipe output failed: %1").arg(formatWin32Error()));
            return;
        }
        ...
        if (FAILED(hr))
        {
            ...
            emitError(QStringLiteral("CreatePseudoConsole failed: %1").arg(formatHresult(hr)));
            return;
        }

        m_inputWrite = hostInputWrite;
        m_outputRead = hostOutputRead;
    ...
    void resize(int cols, int rows)
    {
        ...
        const HRESULT hr = ResizePseudoConsole(m_hPC, size);

        if (FAILED(hr))
        {
            emitError(QStringLiteral("ResizePseudoConsole failed: %1").arg(formatHresult(hr)));
        }
    }
    ...
    void stop()
    {
...

        {
            ...
            if (m_inputWrite && m_inputWrite != INVALID_HANDLE_VALUE)
            {
        ...
        if (m_outputRead && m_outputRead != INVALID_HANDLE_VALUE)
        {
...
signals:
    void outputBase64(const QString &base64);
...
private:
    void writeBytes(const QByteArray &bytes)
    {
        ...
        if (!m_running.load() || !m_inputWrite || m_inputWrite == INVALID_HANDLE_VALUE)
        {
            return;
        }
        ...
        if (!ok)
        {
            emitError(QStringLiteral("WriteFile failed: %1").arg(formatWin32Error()));
        }
    }

    void emitOutput(const QByteArray &raw)
    {
        const QString base64 = QString::fromLatin1(raw.toBase64());
    ...
    bool launchPowerShell()
    {
        if (!m_hPC)
        {
            emitError(QStringLiteral("Pseudo console is not initialized."));
            return false;
        }

        QString exe = QStandardPaths::findExecutable(QStringLiteral("pwsh.exe"));

        if (exe.isEmpty())
        {
            exe = QStandardPaths::findExecutable(QStringLiteral("powershell.exe"));
        }
        ...
        if (exe.isEmpty())
        {
            ...
            exe = systemRoot + QStringLiteral("\\System32\\WindowsPowerShell\\v1.0\\powershell.exe");
        }
        ...
        {
            emitError(QStringLiteral("PowerShell executable not found: %1").arg(exe));
            return false;
        }
        ...
        if (attrListSize == 0)
        {
            emitError(QStringLiteral("InitializeProcThreadAttributeList size query failed: %1")
                          .arg(formatWin32Error()));
            return false;
        }

        std::vector<char> attrList(attrListSize);
        si.lpAttributeList =
            reinterpret_cast<LPPROC_THREAD_ATTRIBUTE_LIST>(attrList.data());

        if (!InitializeProcThreadAttributeList(si.lpAttributeList, 1, 0, &attrListSize))
        {
            emitError(QStringLiteral("InitializeProcThreadAttributeList failed: %1")
                          .arg(formatWin32Error()));
            return false;
        }

        const BOOL attrOk = UpdateProcThreadAttribute(
            si.lpAttributeList,
            0,
        ...
        if (!attrOk)
        {
            ...
            emitError(QStringLiteral("UpdateProcThreadAttribute failed: %1")
                          .arg(formatWin32Error()));
            return false;
        }
        ...
        QVector<wchar_t> cmd = toWideBuffer(commandLine);

        const QString nativeStartDirectory = QDir::toNativeSeparators(QDir::cleanPath(QDir::homePath()));
        QVector<wchar_t> cwd = toWideBuffer(nativeStartDirectory);
        ...
        if (!created)
        {
            emitError(QStringLiteral("CreateProcessW failed: %1").arg(formatWin32Error()));
            return false;
        }
    ...
    void readLoop()
    {
        ...
        while (m_running.load())
        {
            ...
            const QByteArray raw(buffer.constData(), static_cast<int>(bytesRead));
            emitOutput(raw);
        }
...
private:
    const int m_windowsBuild = 19045;
    QString m_initialWorkingDirectory = QDir::homePath();
...
<body>
<div id="terminal"></div>
<script src="qrc:///web/xterm.js"></script>
<script src="qrc:///web/addon-fit.js"></script>
<script src="qrc:///web/addon-web-links.js"></script>
<script src="qrc:///web/addon-unicode11.js"></script>
...
(() => {
    ...
    function sendBytes(bytes)
    {
        ...
        bridge.writeBase64(bytesToBase64(bytes));
    }
    ...
    function resolveCtor(globalName, className)
    {
        if (window[globalName] && window[globalName][className])
        {
            return window[globalName][className];
        }
        if (window[className])
        {
            return window[className];
        }
        throw new Error('Missing xterm addon constructor: ' + globalName + '.' + className);
    }

    function safeCols()
    {
        if (!term || !Number.isFinite(term.cols))
        {
            return MIN_COLS;
        }
        return Math.max(MIN_COLS, term.cols);
    }

    function safeRows()
    {
        if (!term || !Number.isFinite(term.rows))
        {
            return MIN_ROWS;
        }
        return Math.max(MIN_ROWS, term.rows);
    }
    ...
    function fitTerminal()
    {
        ...
        {
            fitAddon.fit();
            if (started && resizeUnlocked && bridge)
            {
                bridge.resize(safeCols(), safeRows());
            }
        }
        catch (e)
        {
    ...
    function applyTheme(dark)
    {
        ...
        const styleId = 'dynamic-terminal-style';
        ...
        styleEl.textContent = `
            #terminal .xterm-viewport {
                background-color: ${viewportBg} !important;
            }
            #terminal .xterm-viewport::-webkit-scrollbar-track {
    ...
    function installKeyboardPolicy()
    {
        term.attachCustomKeyEventHandler(event => {
            ...
            if (shift && !ctrl && !alt && event.key === 'Insert') {
                pasteClipboard();
                event.preventDefault();
                return false;
            }
    ...
    function createTerminal(windowsBuild)
    {
        term = new Terminal({
            cols: MIN_COLS,
            rows: 10,
            ...
            scrollback: 10000,
            tabStopWidth: 4,
            windowsPty: { backend: 'conpty', buildNumber: windowsBuild || 19045 },
        ...
        try {
            const Unicode11AddonCtor = resolveCtor('Unicode11Addon', 'Unicode11Addon');
            term.loadAddon(new Unicode11AddonCtor());
        } catch (e) { console.warn('Unicode11 addon not loaded:', e); }

        const FitAddonCtor = resolveCtor('FitAddon', 'FitAddon');
        fitAddon = new FitAddonCtor();
        term.loadAddon(fitAddon);

        try {
            const WebLinksAddonCtor = resolveCtor('WebLinksAddon', 'WebLinksAddon');
            term.loadAddon(new WebLinksAddonCtor((event, uri) => {
                event.preventDefault();
                if (bridge) bridge.openUrl(uri);
            }));
        } catch (e) { console.warn('web-links addon not loaded:', e); }
        ...
        installKeyboardPolicy();

        bridge.outputBase64.connect(base64 => {
        ...
        bridge.clipboardTextReceived.connect(text => {
        ...
        bridge.processExited.connect(code => {
            term.writeln('');
            term.writeln('\x1b[33m[PowerShell exited with code ' + code + ']\x1b[0m');
        });
        ...
        term.onResize(size => {
            if (started && resizeUnlocked && bridge) {
                bridge.resize(Math.max(MIN_COLS, size.cols), Math.max(MIN_ROWS, size.rows));
            }
        });
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                setTimeout(() => {
                    fitTerminal();
                    bridge.start(safeCols(), safeRows());
                    started = true;
                    term.focus();
                    setTimeout(() => { resizeUnlocked = true; }, 1200);
                }, 120);
            });
        });
    }
    ...
    function boot()
    {
        window.CompilerTerminalApplyTheme = applyTheme;
        window.CompilerTerminalClear = clearTerminal;
        window.CompilerTerminalFocus = () => { if (term) term.focus(); };

        if (!window.Terminal) {
            document.body.innerHTML = '<pre style="color:red;background:#0c0c0c;margin:0;padding:12px;">xterm.js is not loaded. Check qrc:///web/xterm.js</pre>';
            return;
        }
        if (!window.QWebChannel) {
            document.body.innerHTML = '<pre style="color:red;background:#0c0c0c;margin:0;padding:12px;">qwebchannel.js is not loaded.</pre>';
            return;
        }
        if (!window.qt || !qt.webChannelTransport) {
            document.body.innerHTML = '<pre style="color:red;background:#0c0c0c;margin:0;padding:12px;">QWebChannel transport is not available.</pre>';
            return;
        }

        new QWebChannel(qt.webChannelTransport, channel => {
            bridge = channel.objects.pty;
            if (!bridge) {
                document.body.innerHTML = '<pre style="color:red;background:#0c0c0c;margin:0;padding:12px;">C++ object "pty" is not registered.</pre>';
                return;
            }
            const build = Number(bridge.windowsBuild || 19045);
            createTerminal(build);
        });
    }
...
private:
    ...
    void onDisplayFontSizeChanged(int size);

    QWebEngineView *view = nullptr;
    QWebEngineProfile *profile = nullptr;
    OfflineRequestInterceptor *interceptor = nullptr;
    TerminalWebPage *page = nullptr;
    ...
    void initCompleter();
    void fetchCompletions(const QString &prefix);
    void startProcess();
    void restartProcessForWorkingDirectory();
    void sendCommand(const QString &cmd);
    void ensureRunning();
    void handleCd(const QString &cmd);

    void queueOrSend(const QString &text);
    void flushPendingCommands();
    void runTerminalScript(const QString &script);
    void applyThemeToWebView();
};

IntegratedTerminal::IntegratedTerminal(QWidget *parent)
    : QDockWidget(tr("终端"), parent)
{
    ...
    interceptor = new OfflineRequestInterceptor(this);
    ...
    pty = new PowerShellPty(this);
    channel = new QWebChannel(this);
    channel->registerObject(QStringLiteral("pty"), pty);

    page = new TerminalWebPage(profile, this);
...
IntegratedTerminal::~IntegratedTerminal()
{
    ...
    if (channel)
    {
        channel->deregisterObject(pty);
    }
}

void IntegratedTerminal::closeEvent(QCloseEvent *event)
{
    saveHistory();
    QDockWidget::closeEvent(event);
}

void IntegratedTerminal::startProcess()
{
    ...
    if (pty)
    {
        pty->setInitialWorkingDirectory(QDir::homePath());
    }

    page->setHtml(terminalHtml(), QUrl(QStringLiteral("qrc:///")));
}

void IntegratedTerminal::restartProcessForWorkingDirectory()
{
    if (pty)
    {
        pty->setInitialWorkingDirectory(QDir::homePath());
...
void IntegratedTerminal::ensureRunning()
{
...
void IntegratedTerminal::sendCommand(const QString &cmd)
{
    queueOrSend(cmd);
}
...
void IntegratedTerminal::executeCommand(const QString &command)
{
    ...
    sendCommand(command + QStringLiteral("\r"));
    handleCd(command);
}
...
void IntegratedTerminal::onProcessFinished(int exitCode, QProcess::ExitStatus status)
{
    Q_UNUSED(exitCode)
    Q_UNUSED(status)
}

void IntegratedTerminal::onProcessError()
{
}

void IntegratedTerminal::onInputReturnPressed()
{
}

void IntegratedTerminal::loadHistory()
{
    QSettings settings("CompilerIDE", "Compiler IDE");
    history = settings.value("TerminalHistory").toStringList();
    if (history.size() > MAX_HISTORY)
    {
        history = history.mid(0, MAX_HISTORY);
    }
    historyIndex = -1;
}

void IntegratedTerminal::saveHistory()
{
    QSettings settings("CompilerIDE", "Compiler IDE");
    settings.setValue("TerminalHistory", history);
}

void IntegratedTerminal::rememberCommand(const QString &command)
{
...
void IntegratedTerminal::initCompleter()
{
}

void IntegratedTerminal::fetchCompletions(const QString &prefix)
{
    Q_UNUSED(prefix)
}

void IntegratedTerminal::onCompleterActivated(const QString &completion)
{
    Q_UNUSED(completion)
}

bool IntegratedTerminal::eventFilter(QObject *obj, QEvent *event)
{
    Q_UNUSED(obj)
    Q_UNUSED(event)
    return false;
...
void IntegratedTerminal::onDisplayFontSizeChanged(int size)
{
    Q_UNUSED(size)
}

void IntegratedTerminal::handleCd(const QString &cmd)
{
    ...
    if (lower == QStringLiteral("cd") || lower == QStringLiteral("set-location") || lower == QStringLiteral("sl"))
    {
        target = QDir::homePath();
    }
    else if (lower.startsWith(QStringLiteral("cd ")))
    {
        target = trimmed.mid(3).trimmed();
    }
    else if (lower.startsWith(QStringLiteral("set-location ")))
    {
        target = trimmed.mid(QStringLiteral("set-location ").size()).trimmed();
    }
    else if (lower.startsWith(QStringLiteral("sl ")))
    {
        target = trimmed.mid(3).trimmed();
    }
    else
    ...
    while ((target.startsWith(QLatin1Char('"')) && target.endsWith(QLatin1Char('"'))) ||
    ...
    if (dir.isRelative())
    {
        dir = QDir(currentDir.isEmpty() ? QDir::currentPath() : currentDir);
        dir.cd(target);
    }

    currentDir = QDir::cleanPath(dir.absolutePath());
}

void IntegratedTerminal::queueOrSend(const QString &text)
{
    if (text.isEmpty())
    {
        return;
...
void IntegratedTerminal::flushPendingCommands()
{
    if (!pty || !pty->isRunning())
    {
        return;
    ...
    const QStringList commands = pendingCommands;
    pendingCommands.clear();

    for (const QString &command : commands)
    {
        pty->writeText(command);
    }
}

void IntegratedTerminal::runTerminalScript(const QString &script)
{
    if (!page || script.isEmpty())
    {
        return;
    }
...
void IntegratedTerminal::applyThemeToWebView()
{
    if (!page)
    {
        return;
...
private slots:
    ...
    void onSha256Fetched();

private:
    void fetchUpdateInfo();
    void showUpdateDialogWithInfo(const QString &updateInfo);
    void startDownload();
    void setupAutoUpdate();
    void fetchExpectedSha256();
    void verifyAndInstall(const QString &filePath);
...
void UpdateManager::checkForUpdates(bool silent)
{
    ...
    QUrl versionUrl("https://compileride.cn/version.txt");
...
void UpdateManager::fetchDownloadUrl()
{
    if (urlReply)
    {
        urlReply->abort();
    ...
    QUrl url("https://compileride.cn/download_URL.txt");
...
void UpdateManager::onUrlCheckFinished()
{
    ...
    {
        urlRetryCount++;
        qDebug() << "下载链接获取失败，重试一次..." << urlReply->errorString();
        fetchDownloadUrl();
        return;
    }

    qDebug() << "重试后仍失败，使用备用 GitHub Release 链接";
    downloadUrl = "https://github.com/CompilerIDE/CompilerIDE.github.io/releases/download/CompilerIDE/CompilerIDE_setup.exe";
    qDebug() << "备用链接:" << downloadUrl;
...
{
    QUrl infoUrl("https://compileride.cn/Update_info.txt");
...
{
    QUrl sha256Url("https://compileride.cn/security_SHA256.json");
...
void UpdateManager::onSha256Fetched()
{
    ...
    QByteArray jsonData = sha256Reply->readAll();
    ...
    expectedSha256 = root.value("CompilerIDE_setup.exe").toString();
    ...
    qDebug() << "获取到期望 SHA256:" << expectedSha256;
    sha256Reply->deleteLater();
    sha256Reply = nullptr;
...
void UpdateManager::onDownloadFinished()
{
    ...
    qDebug() << "下载完成，文件大小:" << QFileInfo(downloadedFilePath).size() << "字节";

    verifyAndInstall(downloadedFilePath);
}

void UpdateManager::verifyAndInstall(const QString &filePath)
{
    QFile file(filePath);
    ...
    if (hash.addData(&file))
    {
        ...
        qDebug() << "实际 SHA256:" << actualSha256;
        qDebug() << "期望 SHA256:" << expectedSha256;
    ...
    if (progressDialog)
    {
        progressDialog->setLabelText(tr("校验通过，准备更新..."));
    ...
    for (int i = 0; i < pageNames.size(); ++i)
    {
        if (pageNames[i] == name)
        {
            ...
            if (!enabled)
            {
                ...
                btn->setChecked(false);
                btn->setStyleSheet("QPushButton { opacity: 0.5; }");
            }
            else
            {
                btn->setCheckable(true);
    ...
    <div class="markdown-body">
        ...
        <div class="source-notice" role="note" aria-label="来源与使用声明">
            ...
            <div>
                ...
                <div class="source-notice-text">
                    题目内容来源于洛谷平台，已获平台方知情。仅做本地浏览，评测请至
                    <a href="https://www.luogu.com.cn/" target="_blank" rel="noopener noreferrer">luogu.com.cn</a>
                    提交。
                </div>
            </div>
        </div>
...
private:
    ...
    void sendChangeToLSP(CodeEditor *editor);
    void sendCloseToLSP(CodeEditor *editor);
    void ensureCompileFlags(const QString &dirPath, bool isEasyX);
...
void CompilerIDE::createMenus()
{
    ...
    QAction *openSourceNoticeAct = new QAction(tr("开源项目使用说明"), this);
    openSourceNoticeAct->setStatusTip(tr("查看本软件使用到的开源组件及其许可证"));
    connect(openSourceNoticeAct, &QAction::triggered, this, &CompilerIDE::showOpenSourceNotice);
    helpMenu->addAction(openSourceNoticeAct);
...
void CompilerIDE::warmupAndRunTest()
{
    ...
    QTimer *warmupTimeout = new QTimer(this);
    warmupTimeout->setSingleShot(true);

    QPointer<QProcess> warmupGuard = warmupProcess;
    QPointer<QTimer> timeoutGuard = warmupTimeout;

    warmupProcess->setProperty("formalTestStarted", false);

    auto startFormalTest = [this, warmupGuard]()
    {
        if (!warmupGuard)
        {
            return;
        ...
        if (warmupGuard->property("formalTestStarted").toBool())
        {
            return;
        }
    ...
    {
        warmupProcess->write(inputData.toLocal8Bit());
        warmupProcess->closeWriteChannel();
        warmupTimeout->start(2000);
    }
    else
    {
        warmupTimeout->stop();
...
void CompilerIDE::onTestFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus != QProcess::NormalExit || exitCode != 0)
    {
        ...
        else if (actualOutput.trimmed().isEmpty())
        {
            actualOutput = tr("运行时错误 (RE)，退出码: %1").arg(exitCode);
        }
    }
    else if (elapsedMs > timeLimit + 10)
    {
        status = "TLE";
        actualOutput = tr("超时");
    }
    ...
    QProcess *finishedProcess = testProcess;
    testProcess = nullptr;
...
void CompilerIDE::onTestTimeout()
{
    ...
    if (testProcess->state() == QProcess::NotRunning)
    {
        ...
        QProcess::ExitStatus exitStatus = testProcess->exitStatus();
        onTestFinished(exitCode, exitStatus);
        return;
    }
...
void CompilerIDE::newFile()
{
    ...
    if (m_lspAvailable && m_lspClient)
    {
        QString tempFilePath = getTempFilePathForNewEditor(editor);
        ...
        QString dir = QFileInfo(tempFilePath).absolutePath();
        bool isEasyXFlag = editorEasyXFlags.value(editor, false);
...
void CompilerIDE::about()
{
    ...
    aboutDialog.setMaximumHeight(480);
    aboutDialog.setMinimumHeight(480);
...
void CompilerIDE::documentWasModified()
{
    ...
    if (changedDoc)
    {
        for (int i = 0; i < tabWidget->count(); i++)
        {
            CodeEditor *e = editorAt(i);
            if (e && e->document() == changedDoc)
            {
                editor = e;
                break;
            }
    ...
    if (m_lspAvailable && m_lspClient && m_editorUris.contains(editor))
    {
...
void CompilerIDE::closeTab(int index)
{
    ...
    if (editor)
    {
        ...
        editorEncodings.remove(editor);
    }

    QString lspDirToClean;
    QString lspTempFileToRemove;

    if (editor)
    {
        if (m_pendingLSPEditor == editor)
        {
            m_pendingLSPEditor = nullptr;
            if (m_lspDebounceTimer && m_lspDebounceTimer->isActive())
            {
        ...
        if (!filePath.isEmpty() && QFile::exists(filePath))
        {
            lspDirToClean = QFileInfo(filePath).absolutePath();
        }
        else if (m_editorUris.contains(editor))
        {
            QString uri = m_editorUris.value(editor);
            QString tempPath = QUrl(uri).toLocalFile();
            if (!tempPath.isEmpty())
            {
                lspDirToClean = QFileInfo(tempPath).absolutePath();
                lspTempFileToRemove = tempPath;
            }
        }
    ...
    if (!lspDirToClean.isEmpty())
    {
        bool otherTabUsesDir = false;
        QDir cleanDir(lspDirToClean);
        QString canonicalCleanDir = cleanDir.canonicalPath();

        for (int i = 0; i < tabWidget->count(); i++)
        {
            CodeEditor *otherEditor = editorAt(i);
            if (!otherEditor)
            {
                continue;
            }

            QString otherFilePath = tabWidget->tabToolTip(i);
            QString otherDir;

            if (!otherFilePath.isEmpty() && QFile::exists(otherFilePath))
            {
                otherDir = QFileInfo(otherFilePath).absolutePath();
            }
            else if (m_editorUris.contains(otherEditor))
            {
                QString otherUri = m_editorUris.value(otherEditor);
                QString otherTempPath = QUrl(otherUri).toLocalFile();
                if (!otherTempPath.isEmpty())
                {
                    otherDir = QFileInfo(otherTempPath).absolutePath();
                }
            }

            if (!otherDir.isEmpty())
            {
                if (QDir(otherDir).canonicalPath() == canonicalCleanDir)
                {
                    otherTabUsesDir = true;
                    break;
                }
            }
        }
        ...
        if (!otherTabUsesDir)
        {
            ...
            if (QFile::exists(metaFilePath))
            {
                ...
                {
                    qDebug() << "[LSP Cleanup] 已删除 meta 文件:" << metaFilePath;
                }
                else
                {
                    qWarning() << "[LSP Cleanup] 无法删除 meta 文件:" << metaFilePath;
                }

                if (QFile::exists(flagsFilePath))
                {
                    if (QFile::remove(flagsFilePath))
                    {
                        qDebug() << "[LSP Cleanup] 已删除 compile_flags.txt:" << flagsFilePath;
                    }
                    else
                    {
                        qWarning() << "[LSP Cleanup] 无法删除 compile_flags.txt:" << flagsFilePath;
                    }
                }
            }
        }
        ...
        {
            qDebug() << "[LSP Cleanup] 目录仍被其他 tab 使用，跳过清理:" << lspDirToClean;
        }
    }

    if (!lspTempFileToRemove.isEmpty() && QFile::exists(lspTempFileToRemove))
    {
        if (QFile::remove(lspTempFileToRemove))
        {
            qDebug() << "[LSP Cleanup] 已删除临时源文件:" << lspTempFileToRemove;
        }
        else
        {
            qWarning() << "[LSP Cleanup] 无法删除临时源文件:" << lspTempFileToRemove;
        }
    }
...
bool CompilerIDE::isCompilerValid(const QString &path)
{
    ...
    QString found = QStandardPaths::findExecutable(p);
    return !found.isEmpty();
}

QStringList CompilerIDE::detectMinGWIncludePaths(const QString &compilerPath)
{
    QStringList paths;

    auto normalizePath = [](QString p) -> QString
    {
        p = p.trimmed();
    ...
    auto addPath = [&paths, &normalizePath](const QString &p)
    {
        QString clean = normalizePath(p);
    ...
    QString resolvedCompilerPath = compilerPath.trimmed();

    if (resolvedCompilerPath.startsWith("\"") &&
        resolvedCompilerPath.endsWith("\"") &&
        resolvedCompilerPath.length() >= 2)
    {
        resolvedCompilerPath = resolvedCompilerPath.mid(1, resolvedCompilerPath.length() - 2);
    }

    QFileInfo compilerInfo(resolvedCompilerPath);

    if (!compilerInfo.exists())
    {
        QString found = QStandardPaths::findExecutable(resolvedCompilerPath);
        if (!found.isEmpty())
        {
            resolvedCompilerPath = found;
            compilerInfo = QFileInfo(resolvedCompilerPath);
        }
    }

    if (!compilerInfo.exists())
    {
        qWarning() << "Compiler not found when detecting include paths:" << compilerPath;
        return paths;
    }

    {
        QProcess proc;
        proc.setProgram(resolvedCompilerPath);
        proc.setArguments(QStringList() << "-E" << "-x" << "c++" << "-" << "-v");
        ...
        if (proc.waitForStarted(3000))
        {
            ...
            {
                QByteArray rawOutput = proc.readAllStandardError() + proc.readAllStandardOutput();

#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
                QString output = QString::fromLocal8Bit(rawOutput);
#else
                QString output = QTextCodec::codecForLocale()->toUnicode(rawOutput);
#endif
                ...
                for (QString line : lines)
                {
                    ...
                    if (inSearchList && line.contains("End of search list."))
                    {
                        break;
                    }
                    ...
                    if (line.startsWith("ignoring ", Qt::CaseInsensitive))
                    {
                        continue;
                    }
            ...
            {
                qWarning() << "Compiler include path query timed out:" << resolvedCompilerPath;
                proc.kill();
        ...
        {
            qWarning() << "Failed to start compiler for include path query:" << resolvedCompilerPath;
        }
    }

    QString compilerDir = compilerInfo.absolutePath().replace("\\", "/");
    QDir rootDir(compilerDir);
    ...
    QDir libGccDir(root + "/lib/gcc");
    if (libGccDir.exists())
    {
        triplets = libGccDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);

        for (const QString &triplet : triplets)
        {
            QDir tripletDir(libGccDir.filePath(triplet));
            QStringList versions = tripletDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);

            for (const QString &version : versions)
            {
                QString gccVersionRoot = tripletDir.filePath(version).replace("\\", "/");

                addPath(gccVersionRoot + "/include");
                addPath(gccVersionRoot + "/include-fixed");

                addPath(gccVersionRoot + "/include/c++");
            }
        }
    }

    QDir cxxRoot(root + "/include/c++");
    if (cxxRoot.exists())
    {
        QStringList versions = cxxRoot.entryList(QDir::Dirs | QDir::NoDotAndDotDot);

        for (const QString &version : versions)
        {
            QString versionRoot = cxxRoot.filePath(version).replace("\\", "/");

            addPath(versionRoot);
            addPath(versionRoot + "/backward");

            for (const QString &triplet : triplets)
            {
                addPath(versionRoot + "/" + triplet);
            }

            QDir versionDir(versionRoot);
            QStringList subDirs = versionDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);

            for (const QString &sub : subDirs)
            {
                QString candidate = versionDir.filePath(sub).replace("\\", "/");

                if (QFile::exists(candidate + "/bits/c++config.h"))
                {
                    addPath(candidate);
                }
            }
    ...
    for (const QString &triplet : triplets)
    {
        addPath(root + "/" + triplet + "/include");
    }

    addPath(root + "/x86_64-w64-mingw32/include");
    addPath(root + "/i686-w64-mingw32/include");
    addPath(root + "/mingw32/include");
    addPath(root + "/include");

    {
        QDirIterator it(root,
                        QStringList() << "stdc++.h" << "c++config.h",
                        QDir::Files,
                        QDirIterator::Subdirectories);

        while (it.hasNext())
        {
            QString filePath = it.next().replace("\\", "/");

            QFileInfo fi(filePath);
            QDir bitsDir = fi.dir();

            if (bitsDir.dirName() != "bits")
            {
                continue;
            }

            QDir includeDir = bitsDir;
            includeDir.cdUp();

            QString parent = includeDir.absolutePath().replace("\\", "/");
            addPath(parent);
        }
    }

    qDebug() << "Detected compiler include paths from" << resolvedCompilerPath;
...
void CompilerIDE::ensureCompileFlags(const QString &dirPath, bool isEasyX)
{
    static QMap<QString, bool> scheduledJobs;
    ...
    if (!standard.startsWith("c++", Qt::CaseInsensitive) &&
        !standard.startsWith("gnu++", Qt::CaseInsensitive))
    {
        standard = "c++17";
    }

    QString compilerAbs = compilerPath.trimmed();
    if (compilerAbs.startsWith("\"") && compilerAbs.endsWith("\"") && compilerAbs.length() >= 2)
    {
        compilerAbs = compilerAbs.mid(1, compilerAbs.length() - 2);
    }

    QFileInfo compilerInfo(compilerAbs);
    if (!compilerInfo.exists())
    {
        QString found = QStandardPaths::findExecutable(compilerAbs);
        if (!found.isEmpty())
        {
            compilerAbs = found;
            compilerInfo = QFileInfo(compilerAbs);
        }
    }

    if (!compilerInfo.exists())
    {
        qWarning() << "Compiler invalid, skip generating compile_flags.txt:" << compilerPath;
        return;
    }
    ...
    QString jobKey = dir.absolutePath() + "|" + expectedKey;

    if (scheduledJobs.value(jobKey, false))
    {
        return;
    }

    scheduledJobs[jobKey] = true;

    QString localDirPath = dir.absolutePath();
    QString localCompilerPath = compilerInfo.absoluteFilePath();
    QString localStandard = standard;
    bool localIsEasyX = isEasyX;
    QPointer<CompilerIDE> self(this);
...
void CompilerIDE::initLSP()
{
    if (m_lspClient || property("lspInitScheduled").toBool())
    {
        return;
    }
...
QString CompilerIDE::getTempFilePathForNewEditor(CodeEditor *editor)
{
    ...
    QString tempDir = QDir::temp().absolutePath() + "/CompilerIDE_LSP_Unsaved";
    QDir().mkpath(tempDir);
    QString fileName = QString("unsaved_%1_%2.cpp").arg(QDateTime::currentMSecsSinceEpoch()).arg(++counter);
    return tempDir + "/" + fileName;
}

void CompilerIDE::sendOpenToLSP(CodeEditor *editor, const QString &filePath)
{
    if (editorEasyXFlags.value(editor, false))
    {
        return;
    ...
    QString wantedPath = QFileInfo(filePath).absoluteFilePath();
    if (wantedPath.isEmpty())
    {
        wantedPath = filePath;
    }

    editor->setProperty("lspWantedFilePath", wantedPath);
    ...
    if (editor->property("lspOpenScheduled").toBool())
    {
        return;
    }

    editor->setProperty("lspOpenScheduled", true);
...
void CompilerIDE::sendChangeToLSP(CodeEditor *editor)
{
    ...
    if (!editor || !m_lspAvailable || !m_lspClient)
    {
        qDebug() << "LSP not available, skip change";
        return;
    }

    if (!m_editorUris.contains(editor))
    {
        qDebug() << "Editor not registered in LSP, skip change";
        return;
    }
    ...
    if (uri.isEmpty())
    {
        qDebug() << "Editor LSP uri is empty, skip change";
        return;
    }
...
void CompilerIDE::sendCloseToLSP(CodeEditor *editor)
{
    ...
    m_lspClient->closeDocument(m_editorUris[editor]);
    m_editorUris.remove(editor);
    m_editorVersions.remove(editor);
}
...
void UpdateManager::setupAutoUpdate()
{
    ...
    bool closed = mainWin->close();
    if (!closed)
    {
        qDebug() << "User cancelled update.";
        return;
    }
